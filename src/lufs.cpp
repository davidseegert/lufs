...
void processAudioFile(const fs::path& path, bool showLufs, bool showRange, bool showPeak, bool isDirectoryMode, const fs::path& baseDir) {
    ...
    if (totalFramesRead > 0) {
        ...
        if (showLufs && showRange && showPeak) {
            std::cout << displayPath << std::endl;
            std::cout << "LUFS: " << integratedLoudness << std::endl;
            std::cout << "LRA:    " << loudnessRange << std::endl;
            std::cout << "TP:     " << std::fixed << std::setprecision(1) << (truePeak <= -99.0 ? -99.0 : truePeak) << std::endl << std::endl;
        } else if (showPeak) {
            std::cout << std::fixed << std::setprecision(1);
            if (isDirectoryMode) {
                std::cout << displayPath << " " << (truePeak <= -99.0 ? -99.0 : truePeak) << std::endl;
            } else {
                std::cout << (truePeak <= -99.0 ? -99.0 : truePeak) << std::endl;
            }
        } else if (showLufs && showRange) {
            std::cout << displayPath << std::endl;
            std::cout << "LUFS: " << integratedLoudness << std::endl;
...
int main(int argc, char** argv) {
    ...
    if (fs::is_directory(inputPath)) {
        ...
        for (const auto& filePath : files) {
            processAudioFile(filePath, showLufs, showRange, showPeak, true, inputPath);
        }
    } else {
        processAudioFile(inputPath, showLufs, showRange, showPeak, false, inputPath.parent_path());
    }
